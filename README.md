...
Runs the live monitoring dashboard in mock mode — no camera or microphone required.
...
The dashboard opens at `http://localhost:8501`. Lane counts, detection scores, and emergency alerts all update in real time using simulated data.

> **Note:** The full system (live camera + microphone + model inference) requires local setup with real hardware. See Option B below.
